...
class ExpandingVFTLGuest(ExpandingVFTLParty):
    ...
    def prepare_local_data(self, overlap_batch_range, non_overlap_batch_range, block_indices=None, block_idx=None):
        ...
        print("[DEBUG] Guest X_overlap: {0}; {1}".format(len(X_overlap), X_overlap.shape))
        ...
        print("[DEBUG] Guest Y_overlap: {0}; {1}".format(len(Y_overlap), Y_overlap.shape))
        ...
        self.Y_all_in = torch.tensor(Y_all_ref_block).float()
        self.Y_overlap_in = torch.tensor(Y_overlap).float()
        self.Y_non_overlap_in = torch.tensor(Y_non_overlap).float()

        self.X_all_in = torch.tensor(X_all_ref_block).float()
        self.X_overlap_in = torch.tensor(X_overlap).float()
        self.X_non_overlap_in = torch.tensor(X_non_overlap).float()

        self.Y_all_in_for_est = torch.tensor(Y_all_ref_block).float()
        self.Y_overlap_in_for_est = torch.tensor(Y_overlap).float()
