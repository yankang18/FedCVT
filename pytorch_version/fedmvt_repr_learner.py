...
class AttentionBasedRepresentationEstimator(object):
    ...
    def select_reprs_for_multiclass(self,
        ...
        for repr_w_candidate_lbl in reprs_w_candidate_labels:
            ...
            # fetch fed labels
            condidate_lbl_1 = repr_w_candidate_lbl[-n_class:]
            # fetch guest_lr labels
            condidate_lbl_2 = repr_w_candidate_lbl[-2 * n_class:-n_class]
            # fetch host_lr labels
            condidate_lbl_3 = repr_w_candidate_lbl[-3 * n_class:- 2 * n_class]

            print("[DEBUG] repr", reprs.shape)
            print("[DEBUG] condidate_lbl_1", condidate_lbl_1.shape)
            print("[DEBUG] condidate_lbl_2", condidate_lbl_2.shape)
            print("[DEBUG] condidate_lbl_3", condidate_lbl_3.shape)

            index_1 = torch.argmax(input=condidate_lbl_1)
            index_2 = torch.argmax(input=condidate_lbl_2)
            index_3 = torch.argmax(input=condidate_lbl_3)
            ...
            if to_gather:
                s_condidate_lbl_1 = sharpen(condidate_lbl_1, temperature=0.1)
