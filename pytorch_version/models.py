...
class SoftmaxRegression(nn.Module):
    ...
    def _build_model(self):
        if self.hidden_dim is None:
            print("[INFO] SoftmaxRegressor applies one layer with input_dim: {0}, num_class: {1}".format(self.input_dim,
                                                                                                         self.n_class))
            self.classifier = nn.Sequential(
                nn.Linear(in_features=self.input_dim, out_features=self.n_class),
            )
        else:
            print("[INFO] SoftmaxRegressor applies two layers with hidden_dim: {0}".format(self.hidden_dim))
            self.classifier = nn.Sequential(
