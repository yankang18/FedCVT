...
class VerticalFederatedTransferLearning(object):
    ...
    def __init__(self, vftl_guest: ExpandingVFTLGuest, vftl_host: ExpandingVFTLHost, model_param: FederatedModelParam):
        ...
        self.repr_learner = AttentionBasedRepresentationEstimator()
    ...
    def _build_feature_extraction_with_transfer(self):
        ...
        sharpen_tempature = self.model_param.sharpen_temperature
        ...
        Uh_non_overlap_ested_reprs = self.repr_learner.estimate_host_reprs_for_guest_party(
            Ug_non_overlap_comm,
            Ug_non_overlap_uniq,
            Ug_overlap_uniq,
            Uh_overlap_uniq,
        ...
        Uh_overlap_ested_reprs = self.repr_learner.estimate_host_reprs_for_guest_party(
            Ug_overlap_comm,
            Ug_overlap_uniq,
            Ug_overlap_uniq,
            Uh_overlap_uniq,
        ...
        Ug_non_overlap_ested_reprs_w_lbls, uniq_lbls, comm_lbls = self.repr_learner.estimate_labeled_guest_reprs_for_host_party(
            Uh_non_overlap_comm,
            Uh_non_overlap_uniq,
            Uh_overlap_uniq,
            Ug_overlap_uniq,
        ...
        result_overlap = self.repr_learner.estimate_guest_reprs_n_lbls_for_host_party(Uh_uniq=Uh_overlap_uniq,
                                                                                      ...
                                                                                      sharpen_tempature=sharpen_tempature,
                                                                                      W_hg=W_hg)
        Ug_overlap_ested_soft_lbls, Ug_overlap_ested_reprs = result_overlap

        result_non_overlap = self.repr_learner.estimate_guest_reprs_n_lbls_for_host_party(Uh_uniq=Uh_non_overlap_uniq,
                                                                                          Uh_overlap_uniq=Uh_overlap_uniq,
                                                                                          ...
                                                                                          sharpen_tempature=sharpen_tempature,
                                                                                          W_hg=W_hg)
        self.Uh_overlap_ested_soft_lbls, _ = result_non_overlap

        self.logistic_regressor.predict_lbls_for_reprs(fed_host_non_overlap_reprs)
        y_hat_host_side = self.logistic_regressor.y_hat_host_side
        Ug_non_overlap_estimated_lbls = tf.expand_dims(Ug_non_overlap_estimated_lbls, axis=1)

        print("fed_host_non_overlap_reprs:", fed_host_non_overlap_reprs)
        print("Ug_non_overlap_estimated_lbls:", Ug_non_overlap_estimated_lbls)
        print("y_hat_host_side:", y_hat_host_side)

        reprs_w_condidate_labels = tf.concat([fed_host_non_overlap_reprs, Ug_non_overlap_estimated_lbls, y_hat_host_side], axis=1)
        selected_fed_host_non_overlap_reprs, selected_host_lbls = self.repr_learner.select_reprs_for_biclass(reprs_w_condidate_labels)

        self.original_shape = tf.shape(input=reprs_w_condidate_labels)
        self.selected_shape = tf.shape(input=selected_fed_host_non_overlap_reprs)
        ...
        train_fed_reprs_list = [fed_overlap_reprs, fed_guest_non_overlap_reprs, selected_fed_host_non_overlap_reprs]
        ...
        fed_Y = tf.concat([Y_overlap, Y_guest_non_overlap, selected_host_lbls], axis=0)
        # fed_Y = tf.concat([Y_overlap, Y_guest_non_overlap], axis=0)
        print("fed_Y shape", fed_Y.shape)

        assistant_loss_list = list()
        assistant_loss_list.append(self.get_shared_reprs_loss(Ug_overlap_comm, Uh_overlap_comm))
        guest_uniq_reprs_loss, host_uniq_reprs_loss = self.get_orth_reprs_loss(Ug_all_uniq, Ug_all_comm, Uh_all_uniq, Uh_all_comm)
        ...
        self.Ug_overlap_ested_lbl_loss = self.get_label_estimation_loss(Ug_overlap_ested_soft_lbls, Y_overlap)
        ...
        label_alignment_loss = self.get_alignment_loss(uniq_lbls, comm_lbls)
    ...
    def get_reprs_loss(Ug_overlap_uniq, Ug_overlap_comm, Uh_overlap_uniq, Uh_overlap_comm, W_hg=None):
        ...
        # guest_uniq_reprs_loss = tf.nn.l2_loss(tf.matmul(Ug_overlap_uniq, tf.transpose(Ug_overlap_comm)))
        # host_uniq_reprs_loss = tf.nn.l2_loss(tf.matmul(Uh_overlap_uniq, tf.transpose(Uh_overlap_comm)))
        if W_hg is None:
            comm_reprs_loss = tf.nn.l2_loss(Ug_overlap_comm - Uh_overlap_comm)
        else:
    ...
    def get_shared_reprs_loss(Ug_overlap_comm, Uh_overlap_comm, W_hg=None):
        if W_hg is None:
            shared_reprs_loss = tf.nn.l2_loss(Ug_overlap_comm - Uh_overlap_comm)
        else:
            transformed_Uh = tf.matmul(Uh_overlap_comm, W_hg)
    ...
    def get_orth_reprs_loss(Ug_uniq, Ug_comm, Uh_uniq, Uh_comm):
        guest_uniq_reprs_loss = - tf.nn.l2_loss(Ug_uniq - Ug_comm)
        host_uniq_reprs_loss = - tf.nn.l2_loss(Uh_uniq - Uh_comm)
    ...
    @staticmethod
    def get_alignment_loss(uniq_lbls, comm_lbls):
    ...
    def build(self):
        ...
        self.logistic_regressor.build(input_dim=input_dim)
        ...
        self.logistic_regressor.set_ops(learning_rate=learning_rate, reg_lambda=reg_lambda,
                                        tf_X_in=self.fed_reprs, tf_labels_in=self.fed_Y)
        ...
        # self.logistic_regressor.set_host_side_predict_ops(fed_host_non_overlap_reprs)
    ...
    def fit(self,
        ...
        original_shape, selected_shape, _, fed_reprs, fed_Y, reprs_dist_loss, reg_loss, pred_loss, ob_loss, Uh_overlap_pred_loss = sess.run(
            [self.original_shape, self.selected_shape, self.logistic_regressor.e2e_train_op, self.fed_reprs, self.fed_Y, self.assistant_loss_list[0],
             self.logistic_regressor.regularization_loss, self.logistic_regressor.pred_loss, self.logistic_regressor.loss, self.Ug_overlap_ested_lbl_loss],
    ...
    def train(self):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for i in range(epoch):
                ...
                # curr_ep = float(i)
                # block_size = init_block_size + int((curr_ep / epoch) * 5000)
