...
class VerticalFederatedTransferLearning(object):
    ...
    def __init__(self,
                 vftl_guest: ExpandingVFTLGuest,
                 vftl_host: ExpandingVFTLHost,
                 model_param: FederatedModelParam,
                 debug=False):
    ...
    def _interact(self):
        ...
        using_uniq = self.model_param.using_uniq
        using_comm = self.model_param.using_comm

        Ug_all, Ug_non_overlap_, Ug_overlap_ = self.vftl_guest.fetch_feat_reprs()
        Uh_all, Uh_non_overlap_, Uh_overlap_ = self.vftl_host.fetch_feat_reprs()

        Ug_all_uniq, Ug_all_comm = Ug_all
        Ug_non_overlap_uniq, Ug_non_overlap_comm = Ug_non_overlap_
        Ug_overlap_uniq, Ug_overlap_comm = Ug_overlap_

        Uh_all_uniq, Uh_all_comm = Uh_all
        Uh_non_overlap_uniq, Uh_non_overlap_comm = Uh_non_overlap_
        Uh_overlap_uniq, Uh_overlap_comm = Uh_overlap_
        ...
        self.Ug_non_overlap_ested_reprs_w_lbls = self.repr_estimator.estimate_labeled_guest_reprs_for_host_party(
            Uh_non_overlap_comm,
            Uh_non_overlap_uniq,
            Uh_overlap_uniq,
            Ug_overlap_uniq,
        ...
        nl_ested_lbls_for_att_ested_guest_reprs = self.Ug_non_overlap_ested_reprs_w_lbls[:, -self.n_class:]

        # # representations for overlapping samples
        # fed_ol_reprs = tf.concat([Ug_overlap_uniq, Ug_overlap_comm, Uh_overlap_uniq, Uh_overlap_comm], axis=1)
        #
        # # representations for non-overlapping samples of guest with host's representations estimated.
        # fed_nl_w_host_ested_reprs = tf.concat([Ug_non_overlap_uniq, Ug_non_overlap_comm, Uh_non_overlap_ested_reprs],
        #                                       axis=1)
        #
        # # representations for non-overlapping samples of host with guest's representations estimated.
        ...
        nl_ested_lbls_for_fed_ested_guest_reprs = F.softmax(self.fed_lr(fed_nl_w_guest_ested_reprs), dim=1)

        # estimate labels for estimated feature representations of missing samples of guest
        nl_ested_lbls_for_lr_ested_guest_reprs = F.softmax(self.guest_lr(Ug_non_overlap_ested_reprs), dim=1)

        # estimate labels for feature representations of non-overlapping samples of host
        nl_ested_lbls_for_lr_host_reprs = F.softmax(self.host_lr(Uh_non_overlap_reprs), dim=1)

        nl_ested_lbls_for_fed_ested_guest_reprs = sharpen(nl_ested_lbls_for_fed_ested_guest_reprs,
                                                          temperature=label_prob_sharpen_temperature)
        nl_ested_lbls_for_lr_ested_guest_reprs = sharpen(nl_ested_lbls_for_lr_ested_guest_reprs,
                                                         temperature=label_prob_sharpen_temperature)
        nl_ested_lbls_for_lr_host_reprs = sharpen(nl_ested_lbls_for_lr_host_reprs,
                                                  temperature=label_prob_sharpen_temperature)

        print("[DEBUG] fed_nl_w_guest_ested_reprs shape:", fed_nl_w_guest_ested_reprs.shape)

        if self.debug: analyze_estimated_labels(nl_ested_lbls_for_att_ested_guest_reprs,
        ...
        self.fed_nl_w_ested_guest_reprs_n_candidate_labels = torch.cat(
            [fed_nl_w_guest_ested_reprs,
             nl_ested_lbls_for_lr_host_reprs,
             nl_ested_lbls_for_lr_ested_guest_reprs,
             nl_ested_lbls_for_fed_ested_guest_reprs], dim=1)

        print("[DEBUG] fed_nl_w_ested_guest_reprs_n_candidate_labels shape {0}".format(
            self.fed_nl_w_ested_guest_reprs_n_candidate_labels.shape))

        # select representations from fed_nl_w_ested_guest_reprs_n_candidate_labels based on the two estimated labels.
        ...
        if has_selected_samples:
            ...
            fed_reprs = torch.cat([fed_ol_reprs, fed_nl_w_host_ested_reprs, sel_fed_nl_w_guest_ested_reprs], dim=0)

            # g_ol_reprs = torch.cat(Ug_overlap_reprs, dim=1)
            # g_nl_reprs = torch.cat(Ug_non_overlap_reprs, dim=1)
            # g_nl_estd_reprs = sel_fed_nl_w_guest_ested_reprs[:, :g_reprs_dim]

            g_ol_reprs = Ug_overlap_reprs
            g_nl_reprs = Ug_non_overlap_reprs
            g_reprs = torch.cat((g_ol_reprs, g_nl_reprs), dim=0)
            # g_reprs = tf.concat((g_ol_reprs, g_nl_reprs, g_nl_estd_reprs), axis=0)

            h_ol_reprs = Uh_overlap_reprs
            ...
            guest_y = torch.cat([Y_overlap, Y_guest_non_overlap], dim=0)
            y = torch.cat([Y_overlap, Y_guest_non_overlap, sel_labels], dim=0)

            train_fed_reprs, train_guest_reprs, train_host_reprs, train_fed_y, train_g_y, train_h_y = \
                fed_reprs, g_reprs, h_ol_reprs, y, guest_y, Y_overlap
            ...
            fed_reprs = torch.cat([fed_ol_reprs, fed_nl_w_host_ested_reprs], dim=0)
            ...
            y = torch.cat([Y_overlap, Y_guest_non_overlap], dim=0)
            train_fed_reprs, train_guest_reprs, train_host_reprs, train_fed_y, train_g_y, train_h_y = \
                fed_reprs, g_reprs, h_ol_reprs, y, y, Y_overlap

            # 3 do not use representation estimation
            # g_ol_reprs = tf.concat(Ug_overlap_reprs, axis=1)
            # h_ol_reprs = tf.concat(Uh_overlap_reprs, axis=1)
            # return fed_ol_reprs, g_ol_reprs, h_ol_reprs, Y_overlap, Y_overlap, Y_overlap
        ...
        if add_assistant_loss:
            ...
            if using_comm:
                assistant_loss_dict['lambda_dist_shared_reprs'] = get_shared_reprs_loss(Ug_overlap_comm,
                                                                                        Uh_overlap_comm,
                                                                                        W_hg=W_hg)
            ...
            if using_uniq:
                Ug_ol_uniq_lbl_loss = get_label_estimation_loss(ol_uniq_lbls, Y_overlap)
                assistant_loss_dict['lambda_host_dist_ested_uniq_lbl_vs_true_lbl'] = Ug_ol_uniq_lbl_loss

            if using_comm:
                Ug_ol_comm_lbl_loss = get_label_estimation_loss(ol_comm_lbls, Y_overlap)
            ...
            Ug_overlap_ested_reprs_alignment_loss = get_alignment_loss(Ug_overlap_ested_reprs, Ug_overlap_reprs)
            ...
            Uh_overlap_ested_reprs_alignment_loss = get_alignment_loss(Uh_overlap_ested_reprs, Uh_overlap_reprs)
    ...
    def build(self):
        ...
        host_input_dim = self.model_param.host_input_dim
        guest_hidden_dim = self.model_param.guest_hidden_dim
        host_hidden_dim = self.model_param.host_hidden_dim
        ...
        self.guest_optimizer = torch.optim.Adam(params=guest_model_params, lr=learning_rate)

        host_model_params = list(self.host_lr.parameters()) + self.vftl_host.get_model_parameters()
        self.host_optimizer = torch.optim.Adam(params=host_model_params, lr=learning_rate)

        fed_model_params = list(
            self.fed_lr.parameters()) + self.vftl_host.get_model_parameters() + self.vftl_guest.get_model_parameters()
        self.fed_optimizer = torch.optim.Adam(params=fed_model_params, lr=learning_rate)
        ...
        #
        #
        # guest_optimizer = torch.optim.Adam(params=self.guest_lr.parameters(), lr=learning_rate)
        # guest_logits = self.guest_lr.forward(guest_reprs)
        # guest_loss = torch.nn.CrossEntropyLoss(guest_logits, guest_Y)
        #
        # host_optimizer = torch.optim.Adam(params=self.host_lr.parameters(), lr=learning_rate)
        # host_logits = self.host_lr.forward(host_reprs)
        # host_loss = torch.nn.CrossEntropyLoss(host_logits, host_Y)
        #
        # fed_model_params = self.fed_lr.parameters()
        # fed_optimizer = torch.optim.Adam(params=fed_model_params, lr=learning_rate)
        # fed_logits = self.fed_lr.forward(self.fed_reprs)
        # fed_objective_loss = torch.nn.CrossEntropyLoss(fed_logits, self.fed_Y)
        #
        ...
        #                       tf_X_in=guest_reprs, tf_labels_in=guest_Y, comb_loss_lambda=1.0)
        ...
        #                      tf_X_in=host_reprs, tf_labels_in=host_Y, comb_loss_lambda=1.0)
        ...
        # self.fed_lr.set_guest_side_predict_ops(fed_nl_w_host_ested_reprs)
    ...
    def _train(self,
        ...
        self.vftl_guest.prepare_local_data(overlap_batch_range=overlap_batch_range,
                                           ...
                                           block_indices=guest_block_indices,
                                           block_idx=guest_block_idx)
        self.vftl_host.prepare_local_data(overlap_batch_range=overlap_batch_range,
    ...
    def train(self):
        ...
        for i in range(epoch):
            ...
            while True:
                ...
                print(f"[DEBUG] ol_block_count/ol_block_num : {ol_block_idx+1}/{ol_block_num}")
                print(f"[DEBUG] ol_train_data/ol_guest(host)_block_size : {ol_end}/{ol_guest_block_size}")
                ...
                # train
                # ==========
                loss_dict = self._train(overlap_batch_range=(ol_start, ol_end),
