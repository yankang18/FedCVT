...
class Autoencoder(FeatureExtractor):
    ...
    def _set_variable_initializer(self):
        ...
        # layer = 0
        # for out_dim in self.hidden_dim_list:
        #     self.enc_layer_vars_initializer_map["We_" + str(layer)] = tf.random_normal((in_dim, out_dim), stddev=np.sqrt(2./(in_dim + out_dim)), dtype=tf.float32)
        #     self.enc_layer_vars_initializer_map["be_" + str(layer)] = np.zeros(out_dim).astype(np.float32)
        #     layer += 1
        #     in_dim = out_dim
        #
        # layer = 0
        # for out_dim in reversed(self.hidden_dim_list[:-1]):
        #     self.dec_layer_vars_initializer_map["Wd_" + str(layer)] = tf.random_normal((in_dim, out_dim), stddev=np.sqrt(2./(in_dim + out_dim)), dtype=tf.float32)
        #     self.dec_layer_vars_initializer_map["bd_" + str(layer)] = np.zeros(out_dim).astype(np.float32)
        #     layer += 1
        #     in_dim = out_dim
        #
        # self.dec_layer_vars_initializer_map["Wd_" + str(layer)] = tf.random_normal((in_dim, self.input_dim), stddev=np.sqrt(2./(in_dim + self.input_dim)), dtype=tf.float32)
        # self.dec_layer_vars_initializer_map["bd_" + str(layer)] = np.zeros(self.input_dim).astype(np.float32)

        # print("self.enc_layer_vars_initializer_map:\n", self.enc_layer_vars_initializer_map)
        # print("self.dec_layer_vars_initializer_map:\n", self.dec_layer_vars_initializer_map)
