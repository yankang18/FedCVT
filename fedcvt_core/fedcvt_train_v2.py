...
class VerticalFederatedTransferLearning(object):
    ...
    def _semi_supervised_training(self):
        ...
        if has_unlabeled_overlapping_samples:
            ...
            # print("Ug_ul_overlap_uniq:", Ug_ul_overlap_uniq)
            # print("Ug_ul_overlap_comm:", Ug_ul_overlap_comm)
        ...
        guest_label_prob_threshold = self.fed_model_param.guest_label_prob_threshold
        ...
        # nl_ested_lbls_use_fed_reprs = sharpen(nl_ested_lbls_use_fed_reprs,
        ...
        #                                               temperature=label_prob_sharpen_temperature)
        # nl_ested_lbls_use_host_reprs = sharpen(nl_ested_lbls_use_host_reprs,
        #                                        temperature=label_prob_sharpen_temperature)
        ...
        if has_selected_samples:
            ...
            print("[DEBUG] num_selected_samples shape:{}".format(selected_reprs_w_lbls.shape))
        ...
        Uh_ol_overlap_ested_reprs = self.repr_estimator.estimate_host_reprs_for_guest_party(
            ...
            # Ug_comm=Ug_ll_overlap_comm,
            # Ug_uniq=Ug_ll_overlap_uniq,
        ...
        result_ll_overlap = self.repr_estimator.estimate_guest_reprs_n_lbls_for_host_party(
            # Uh_comm=Uh_ol_overlap_comm,
            # Uh_uniq=Uh_ol_overlap_uniq,
    ...
    def collaborative_predict(self, data_type="test"):
        # if debug:
        ...
        Ug_overlap_uniq, Ug_overlap_comm, y_true = self.vftl_guest.predict_on_overlap_data(data_type=data_type)
        Uh_overlap_uniq, Uh_overlap_comm = self.vftl_host.predict_on_overlap_data(data_type=data_type)
        ...
        res = precision_recall_fscore_support(y_true_1d, y_hat_1d)
        all_fscore = f_score(res[0], res[1])
        acc = accuracy_score(y_true_1d, y_hat_1d)
        auc = roc_auc_score(y_true, y_prob_two_sides)

        # print("[INFO] all_res:", res)
        # print("[INFO] all_fscore : {0}, all_auc : {1}, all_acc : {2}".format(all_fscore, auc, acc))
        print("[INFO] {} all_auc : {}, all_acc : {}".format(data_type, auc, acc))
    ...
    def train(self):
        ...
        for i in range(epoch):
            ...
            print(ll_guest_block_size, ll_host_block_size)
            ...
            while True:
                ...
                if (iter + 1) % valid_iteration_interval == 0:
                    ...
                    # ====================
                    self.collaborative_predict(data_type="train")

                    all_acc, all_auc, all_fscore = self.collaborative_predict(data_type="test")
